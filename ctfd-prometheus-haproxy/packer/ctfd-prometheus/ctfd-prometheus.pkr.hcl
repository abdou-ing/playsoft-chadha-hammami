...
build {
  sources = ["source.proxmox-clone.ctfd_prometheus"]

  provisioner "ansible" {
    playbook_file = "../../ansible/prometheus.yml"
    extra_arguments = [
      "--extra-vars", "ansible_ssh_pass=${var.ssh_password} ansible_become_pass=${var.ssh_password} ansible_remote_tmp=/home/bob/.ansible_tmp machine1_ip=${var.machine1_ip} machine2_ip=${var.machine2_ip}",
