# VM 210 — Connexion Légitime SSH

source "proxmox-clone" "wazuh-legit" {
  ...
  vm_name              = "wazuh-legit-{{timestamp}}"
  template_description = "Wazuh Lab — Connexion légitime SSH scenario"
...
build {
  name    = "wazuh-legit"
  sources = ["source.proxmox-clone.wazuh-legit"]
...
# 0. NOPASSWD sudo
  ...
  # 1. Upload fichiers
  provisioner "file" {
    source      = "files/config-legit.sh"   
    destination = "/tmp/config.sh"
  }

  provisioner "file" {
    source      = "files/99-static-legit.yaml"  
  ...
  post-processor "shell-local" {
    ...
    inline = [
      ...
      "echo '================================================='",
      "echo ' Wazuh legitime - Build Complete! '",
      "echo '================================================='"
    ]
  }
}
